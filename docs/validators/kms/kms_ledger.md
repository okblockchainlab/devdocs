...
chain_id = "okchain-11001"
reconnect = true # true is the default
secret_key = "~/.tmkms/secret_connection.key"

[[providers.ledgertm]]
chain_ids = ["okchain-11001"]
```

- Edit `addr` to point to your `okchaind` instance.
- Adjust `chain-id` to match your `.okchaind/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `okchaind`.
...
The KMS may complain that it cannot connect to okchaind. That is fine, we will fix it in the next section.
...
## OKChain configuration

You need to enable KMS access by editing `.okchaind/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `okchaind`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell okchaind the key that we've got in the previous section.

```bash
okchaind gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `okchaind`. You should see that the KMS connects and receives a signature request.
