...
# Run a Validator on the OKChain Mainnet
...
Before setting up your validator node, make sure you've already gone through the [Full Node Setup](../okchain-tutorials/join-mainnet.md) guide.
...
You may want to skip the next section if you have already [set up a full-node](../okchain-tutorials/join-mainnet.md).
...
::: warning
The genesis ceremony for the OKChain mainnet is closed. Please skip to the next section.
:::
...
Next, craft your `okchaind gentx` command. 
...
You should now see your validator in one of the OKChain explorers. You are looking for the `bech32` encoded `address` in the `~/.okchaind/config/priv_validator.json` file.
...
you want your node to shutdown or by passing the `--halt-height` flag to `okchaind`.
...
If you got jailed for downtime, you can get your voting power back to your validator. First, if `okchaind` is not running, start it up again:
...
### Problem #2: My `okchaind` crashes because of `too many open files`

The default number of files Linux can open (per-process) is `1024`. `okchaind` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed) and then restart the process with `okchaind start`. If you are using `systemd` or another process manager to launch `okchaind` this may require some configuration at that level. A sample `systemd` file to fix this issue is below:
