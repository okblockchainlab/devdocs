...
Delegators are okt holders who cannot, or do not want to run a validator themselves. okt holders can delegate okts to a validator and obtain a part of their revenue in exchange (for more detail on how revenue is distributed, see [**What is the incentive to stake?**](#what-is-the-incentive-to-stake?) and [**What are validators commission?**](#what-are-validators-commission?) sections below).
...
- **Minimum self-delegation:** Minimum amount of okts the validator needs to have bonded at all time. If the validator's self-delegated stake falls below this limit, their entire staking pool will unbond.
...
- **Tendermint Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `cosmosvalconspub` (Get this value with `okchaind tendermint show-validator`)
  - It is generated when the node is created with okchaind init.
- **Application key**: This key is created from `okchaincli` and used to sign transactions. Application keys are associated with a public key prefixed by `cosmospub` and an address prefixed by `cosmos`. Both are derived from account keys generated by `okchaincli keys add`.
...
- **Amount of self-delegated okts:** Number of okts a validator self-delegated to themselves. A validator with a higher amount of self-delegated okts has more skin in the game, making them more liable for their actions.
...
Staking okts can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an `unbonding` transaction. Then, okts undergo a **3 weeks unbonding period** during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block rewards, fees, and have the right to participate in governance. If a validator misbehaves, a certain portion of their total stake is slashed. This means that every delegator that bonded okts to this validator gets penalized in proportion to their bonded stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.
...
### Will validators of the OKChain ever be required to validate other zones in the OKChain ecosystem?

Yes, they will. If governance decides so, validators of the OKChain hub may be required to validate additional zones in the OKChain ecosystem.
...
- **Block rewards:** Native tokens of applications run by validators (e.g. okts on the OKChain) are inflated to produce block provisions. These provisions exist to incentivize okt holders to bond their stake, as non-bonded okt will be diluted over time.
- **Transaction fees:** The OKChain maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `okt`.
...
Revenue received by a validator's pool is split between the validator and their delegators. The validator can apply a commission on the part of the revenue that goes to their delegators. This commission is set as a percentage. Each validator is free to set their initial commission, maximum daily commission change rate and maximum commission. The OKChain enforces the parameter that each validator sets. Only the commission rate can change after the validator is created.

### How are block rewards distributed?

Block rewards are distributed proportionally to all validators relative to their voting power. This means that even though each validator gains okts with each reward, all validators will maintain equal weight over time.

Let us take an example where we have 10 validators with equal voting power and a commission rate of 1%. Let us also assume that the reward for a block is 1000 okts and that each validator has 20% of self-bonded okts. These tokens do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator's pool has 100 okts. These 100 okts will be distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 okts`
- Validator gets: `100\*20% + Commission = 20.8 okts`
- All delegators get: `100\*80% - Commission = 79.2 okts`

Then, each delegator can claim their part of the 79.2 okts in proportion to their stake in the validator's staking pool.
...
Let's take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission rate and has 20% of self-delegated okts. Now comes a successful block that collects a total of 1025.51020408 okts in fees.

First, a 2% tax is applied. The corresponding okts go to the reserve pool. Reserve pool's funds can be allocated through governance to fund bounties and upgrades.

- `2% * 1025.51020408 = 20.51020408` okts go to the reserve pool.

1005 okts now remain. Let's assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
- For the proposer validator:
  - The pool obtains `R + R * 5%`: 105 okts
  - Commission: `105 * 80% * 1%` = 0.84 okts
  - Validator's reward: `105 * 20% + Commission` = 21.84 okts
  - Delegators' rewards: `105 * 80% - Commission` = 83.16 okts (each delegator will be able to claim its portion of these rewards in proportion to their stake)
- For each non-proposer validator:
  - The pool obtains R: 100 okts
  - Commission: `100 * 80% * 1%` = 0.8 okts
  - Validator's reward: `100 * 20% + Commission` = 20.8 okts
  - Delegators' rewards: `100 * 80% - Commission` = 79.2 okts (each delegator will be able to claim their portion of these rewards in proportion to their stake)
...
### Do validators need to self-delegate okts?

Yes, they do need to self-delegate at least `1 okt`. Even though there is no obligation for validators to self-delegate more than `1 okt`, delegators should want their validator to have more self-delegated okts in their staking pool. In other words, validators should have skin in the game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-delegated okts. If a validator's self-delegation goes below the limit that it predefined, this validator and all of its delegators will unbond.

### How to prevent concentration of stake in the hands of a few top validators?

For now the community is expected to behave in a smart and self-preserving way. When a mining pool in Bitcoin gets too much mining power the community usually stops contributing to that pool. The OKChain will rely on the same effect initially. Other mechanisms are in place to smoothen this process as much as possible:
