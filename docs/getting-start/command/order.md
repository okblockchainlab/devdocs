...
Global Flags:
  ...
  -e, --encoding string   Binary encoding (hex|b64|btc) (default "hex")
      --home string       directory for config and data (default "/Users/oker/.okchaincli")
...
```
# example
okchaincli tx order new --product mycoin_okt -s BUY -p 9.9 -q 2.0 --from alice
# example return
{
...
```
# example
okchaincli tx order new --product mycoin_okt,mycoin2_okt -s BUY,SELL -p 9.9,88 -q 2.0,32.6 --from alice
# example return
{
  "txhash": "57A390CBBE43548F112956DF6CA7146E14A02720F2895908B3A057136CE0324D"
}
...
`--product`表示数字资产交易对名称，用”,“分割多个交易对名称

`-s`表示买卖方向，取值BUY/SELL，用”,“分割多个买卖方向

`-p`表示价格，用”,“分割多个价格

`-q`表示购买或出售的数量，用”,“分割多个数量
...
```bash
okchaincli tx order cancel [order-id]
...
```
`order-id`表示订单Id，这是由应用层自动生成的唯一id
...
```
# example
okchaincli tx order cancel ID0000000054-0000,ID0000000098-0000 --from alice 
...
`order-id`表示订单Id，这是由应用层自动生成的唯一id，多个id用“,“分割开
...
```bash
okchaincli query order detail [order-id]
