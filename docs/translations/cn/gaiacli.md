...
`okchaincli`是一个工具，使您能够与 OKChain 网络中的节点进行交互，无论您是否自己运行它。 让我们恰当的设置它。 要安装它，请按照[安装步骤](./ installation.md)进行安装。

### 配置 okchaincli

设置`okchaincli`的主要命令如下：
...
+ `cosmos` 
	+ 从通过`okchaincli keys add`生成的账户私钥中产生
	+ 用于接收资金
...
+ `cosmosvalconspub`
	+ 在使用`okchaind init`创建节点时生成
	+ 使用`okchaind tendermint show-validator`获得该值
...
在广播交易时，`okchaincli`接受`--broadcast-mode`标识。 这个标识的值可以是`sync`（默认值）、`async`或`block`，其中`sync`使客户端返回 CheckTx 响应，`async`使客户端立即返回，而`block`使得 客户端等待 tx 被提交（或超时）。
...
```bash
okchaincli tx send <destination_cosmos> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> 
```
...
```bash
okchaincli tx send <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --from=<key_name> \
  --dry-run
```
...
一旦主网上线，你可以把`okt`委托给一个验证人。这些委托人可以收到部分验证人的收益。阅读[OKChain Token Model](https://github.com/cosmos/cosmos/raw/master/Cosmos_Token_Model.pdf)了解更多信息。
...
其中`[name]`是初始化`okchaind`时指定的键的名称。
...
治理是Cosmos Hub的用户可以就软件升级，主网的参数或自定义文本提案并达成共识的过程。这是通过对提案进行投票来完成的，提案将由主要网络上的`okt`持有者提交。

关于投票过程的一些考虑因素：
+ 投票由绑定`okt`的持有者以1个绑定的`okt`对应1票方式投出
+ 委托人不投票的话会将票权继承给其验证人
...
在提案的存入金达到`MinDeposit`后，投票期将开放。抵押了`okt`的持有人可以投票：
...
可以通过完全命令生成主流的UNIX shell解释器（如`Bash`和`Zsh`）的`completion`命令，该命令可用于`okchaind`和`okchaincli`。
